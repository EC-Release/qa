...
public class EC_Setup2 extends TestBase {
	...
	public void ecSetup() {
		try {
			...
			createECService[1] = "/c";
			createECService[2] = "cd c:\\ && cd users && cd sareddyc && cd SampleEC && set https_proxy=http://3.28.29.241:88/ && cf create-service enterprise-connect Oneway-TLS sampleec-ecservice43 -c uaa-issuerid.json";
			...
			status_ECService[2] = "cd c:\\ && cd users && cd sareddyc && cd SampleEC && set https_proxy=http://3.28.29.241:88/ && cf service sampleec-ecservice43";
			...
			createECServiceKey_GetInfo[2] = "cd c:\\ && cd users && cd sareddyc && cd SampleEC && set https_proxy=http://3.28.29.241:88/ && cf csk sampleec-ecservice43 sampleec-ecservice43-skey && cf service-key sampleec-ecservice43 sampleec-ecservice43-skey";
...
//	@Test(priority = 4)
	public void ecSetupContinue() throws IOException, ParseException, JSONException, InterruptedException {
		...
		pushGateway[0] = "cmd";
		pushGateway[1] = "/c";
		pushGateway[2] = "cd c:\\ && cd users && cd sareddyc && cd SampleEC && cd Gateway && set https_proxy=http://3.28.29.241:88/ && cf push sampleec-ecservice43-gateway";
