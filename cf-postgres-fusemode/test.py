...
#fuse mode
EC_GW_SERVER_APP_NAME=os.environ['EC_GW_SERVER_INST']
EC_AGT_GW_SERVER_VAR='./'+EC_ART_NAME+' -mod gw:server -zon '+EC_SERVICE_ZONE+' -aid '+EC_SERVER_ID+' -hst wss://'+EC_GATEWAY_APP_NAME+'.'+EC_SHARED_DOMAIN+'/agent -tkn '+EC_SERVICE_TKN+' -rht '+EC_PSQL_HOST+' -gpt ${PORT} -rpt 5432 -cid '+EC_UAA_CID+' -csc '+EC_UAA_CSC+' -oa2 '+EC_UAA+' -dur 300 -dbg -hca ${PORT} -shc -zon '+EC_SERVICE_ZONE+' -sst '+EC_SERVICE+' -grp '+EC_GROUP_ID
...
def main():
    ...
    c.sed(EC_YML,'{APP_NAME}',EC_GW_SERVER_APP_NAME)
    c.sed(EC_YML,'{EXE_FILE}',EC_BATCH)
    c.sed(EC_YML,'{SERVICE_INST}',EC_SERVICE_INST)
    print('gw-server mode cmd-')
    print(EC_AGT_GW_SERVER_VAR)
    c.sed(EC_BATCH,'{EC_AGENT_VAR}',EC_AGT_GW_SERVER_VAR)
    ...
    #scale gateway
    os.system(EC_SCALE_CMD.format(EC_GW_SERVER_APP_NAME, EC_SCALE_NUM))
